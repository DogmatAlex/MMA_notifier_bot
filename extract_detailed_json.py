#!/usr/bin/env python3
"""
Extract detailed JSON data from MatchTV
"""
...
def extract_detailed_json():
    ...
    try:
        ...
        for match in re.finditer(pattern, response_text):
            ...
            logging.info(f"Found pattern match length: {len(matched_text)}")
            ...
            try:
                ...
                logging.info(f"Processing JSON string of length: {len(full_json_str)}")
                ...
                if data and isinstance(data[-1], str):
                    ...
                    data_str = data[-1]
                    logging.info(f"Data string length: {len(data_str)}")
                    ...
                    if null_pos != -1:
                        ...
                        if json_start != -1 and json_end > json_start:
                            ...
                            logging.info(f"JSON part length: {len(json_part)}")
                            ...
                            try:
                                ...
                                for i, channel in enumerate(channels[:3]):  # Show first 3
                                    ...
                                    logging.info(f"Channel {i} {channel_name} has {len(schedule)} schedule items")
                                    
                                    # Show first few schedule items
                                    for item in schedule[:2]:
                                        logging.info(f"  {item.get('time', 'N/A')}: {item.get('title', '')[:50]}...")
                                
                                # Return the data for further processing
                                return channels
                            except json.JSONDecodeError as e:
                                logging.error(f"Error parsing schedule JSON: {e}")
                                logging.error(f"JSON part preview: {json_part[:200]}...")
                        else:
                            logging.error("Could not extract JSON part from data string")
                    else:
                        logging.error("Could not find 'null,' in data string")
                else:
                    logging.error("Unexpected data structure")
                    
            except json.JSONDecodeError as e:
                logging.error(f"Error parsing main JSON: {e}")
                logging.error(f"JSON string preview: {full_json_str[:200]}...")
                continue
            except Exception as e:
        ...
        logging.error(f"Error in detailed JSON extraction: {e}")
        import traceback
        traceback.print_exc()
        return []

def test_extraction():
    """Test the detailed JSON extraction"""
    print("Testing detailed JSON extraction...")
    
    try:
        channels = extract_detailed_json()
